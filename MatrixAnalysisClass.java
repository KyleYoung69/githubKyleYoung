            ...
            int xIndex, int yIndex, int recLevel )
{
    	...
    	String tryLocation = "Trying Location: ";
    	String failLocation = "Location Failed; ";
    	String foundAt = "Found at: ";
    	String validLocationFound = "Valid Location found: ";

    	displayStatus(recLevel, tryLocation, currentCellData, TEST_LOCATION);

    	if(!isInBounds(xIndex, yIndex))
    	{
    		displayStatus(recLevel, failLocation, currentCellData, OUT_OF_BOUNDS);     
    		return false;
    	}
    	...
    	{
    		displayStatus(recLevel, failLocation, currentCellData, INVALID_ITEM);
    		return false;
    	}
    	...
    	{
    		displayStatus(recLevel, failLocation, currentCellData, OVER_SUM);
    		return false;
    	}
    	...
    	// Now it's just successful locations 

    	locationSet.addItem(currentCellData);

    	displayStatus(recLevel, validLocationFound, currentCellData, VALID_ITEM);
    	if(runningTotal == sumRequest && yIndex == arrayRowHeight - 1)
    	{        
    		return true;
    	}

    	if(findSumHelper(sumRequest, runningTotal, xIndex + 1, yIndex,
    			recLevel + 1))
    	{
    		displayStatus(recLevel, foundAt, currentCellData, VALID_ITEM);
    		return true;
    	}

    	else if(findSumHelper(sumRequest, runningTotal, xIndex,
    			yIndex + 1, recLevel + 1))
    	{
    		displayStatus(recLevel, foundAt, currentCellData, VALID_ITEM);
    		return true;
    	...
    	else if(findSumHelper(sumRequest, runningTotal, xIndex - 1,
    			yIndex, recLevel + 1))
    	{
    		displayStatus(recLevel, foundAt, currentCellData, VALID_ITEM);
    		return true;
    	...
    	displayStatus(recLevel, failLocation, currentCellData, NO_SOLUTION);
    	return false;
	}
